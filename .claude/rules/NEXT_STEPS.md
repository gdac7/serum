...
- **Run state lives in PostgreSQL** (`server/db/run_repository.py`, `runs` table). `POST /v1/runs` inserts the run in the same transaction that validates it, and `run_autodan_task` updates status per phase, records `strategies_at_start`/`strategies_loaded`, appends `persist_errors`, and stores the final `phases`/`generations`/`metrics` as a `jsonb` result. `RunRepository` follows the same one-connection-per-unit-of-work pattern as `StrategyRepository`; status transitions are short independent transactions (minutes apart). On startup, `fail_interrupted` reconciles any run left non-terminal by a crash to `failed`. The `users` dict is gone: run ownership is a `client_id` column, target ownership a `client_id` field on the in-memory target. `GET /v1/db_health` reports the run count.
...
2. ~~**Wire the library into the run pipeline**~~ — *done 2026-08-09*, see
   Progress above.
3. ~~**Run state**~~ — *done 2026-08-11*, see Progress above. `runs` and `users`
   moved to Postgres (`users` eliminated entirely). `targets` stays in memory by
   design: the model instance is a live GPU handle that can't be persisted, so a
