...
class AutoDANTurbo:
    ...
    """
    def __init__(self, attacker_model: ModelInterfaceAPI | LocalModel, target_model: ModelInterfaceAPI | LocalModel, scorer_model: ModelInterfaceAPI | LocalModel, summarizer_model: ModelInterfaceAPI, config: Dict[str, Any], on_strategy_discovered: Optional[Callable[[JailbreakStrategy], None]] = None, on_request_completed: Optional[Callable[[Dict[str, Any]], None]] = None):
        ...
        # Called once per malicious request as each finishes, so a caller can
        # report per-request scores while the phase is still running.
    ...
    def _emit_request_scores(self, phase: str, index: int, total: int, request: str, logs: List[AttackLog]) -> None:
        """Report one finished malicious request's scores. Never fatal to a phase."""
        if self.on_request_completed is None:
            return
        scores = [log.score for log in logs if log is not None and log.score is not None]
        entry = {
            "phase": phase,
            "request_index": index,
        ...
        try:
            self.on_request_completed(entry)
        except Exception as e:
            print(f"Failed to report scores for request {index}: {type(e).__name__}: {e}")
        ...
        for i, request in tqdm(enumerate(malicious_request), total=len(malicious_request), desc="Warm-up exploration"):
            ...
            for iteration in range(max_iterations):
                try:
                    ...
                    continue
            summarization_iterations = max_summarization_iterations
            if len(request_logs) < 2:
                print(
                    "Skipping warmup strategy discovery: "
                    f"need at least 2 scored attacks, got {len(request_logs)}"
                )
                summarization_iterations = 0
            summarizaton_pbar = tqdm(range(summarization_iterations), desc=f"Discovering Strategies in Warmup (Req {i+1})", leave=True)
            for _ in summarizaton_pbar:
                ...
                if strategy_j.score >= strategy_i.score:
                    ...
                    if strategy_candidate is not None:
                        ...
                        debug_no_strategy_discovered()
            self._emit_request_scores("warmup", i + 1, len(malicious_request), request, request_logs)
            if DEBUGGING_MODE:
            ...
            for iteration in range(max_iterations):
                try:
                    if previous_attack is None:
                        attack_log = self.attacker_generator.generate_initial_attack(request)
                        if attack_log is None:
                            print(f"Lifelong iteration {iteration + 1} produced no scored attack; retrying")
                            continue
                        self.total_attacks += 1     
                    else:
                        ...
                        if attack_log is None:
                            print(f"Lifelong iteration {iteration + 1} produced no scored attack; skipping")
                            continue
                    ...
                    continue
            self._emit_request_scores("lifelong", i + 1, len(malicious_request), request, request_logs)
            if DEBUGGING_MODE:
                print(f"\nSaving {self.strategy_library.total_strategies} after request {i+1}/{len(malicious_request)}")
    ...
    def evaluate(self, behavior: List[str], num_steps: int, evaluate_mode: bool = True, save_dir: str = "results/evaluate", file_name: str = "generations", **kwargs) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for behavior_desc in tqdm(behavior, desc="Getting target answers"):
            ...
            for i in tqdm(range(num_steps + 1), desc=f"Steps for behavior: {behavior_desc:10}...", leave=False):
                ...
                if attack_log is None:
                    print(f"Evaluation step {i + 1} produced no attack; retrying")
                    continue
                attack_info = {
                    "malicious_request": behavior_desc,
