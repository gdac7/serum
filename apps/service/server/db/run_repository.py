...
def _from_row(row: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "request_scores": list(row["request_scores"] or []),
        "result": row["result"],
    }
...
class RunRepository:
    ...
    def append_request_score(self, run_id: str, entry: Dict[str, Any]) -> None:
        """Append one finished malicious request's score summary.

        Appended rather than rewritten so a run reports partial results while
        it is still going, without holding the whole list in the task.
        """
        with self.conn.cursor() as cur:
            cur.execute(
                """
                UPDATE runs
                SET request_scores = request_scores || %s, updated_at = now()
                WHERE run_id = %s
                """,
                (_as_jsonb([entry]), run_id),
            )
