...
def test_local_target_loading_waits_for_gpu(monkeypatch):
    ...
    monkeypatch.setitem(state.targets, target_id, {"config": {"kind": "local"}})
    gpu_lock = MagicMock()
    build_target = Mock()
    monkeypatch.setattr(state, "gpu_lock", gpu_lock)
    monkeypatch.setattr(state, "build_target", build_target)

    state.load_target(target_id)
...
def test_remote_target_loading_does_not_wait_for_gpu(monkeypatch):
    ...
    monkeypatch.setitem(state.targets, target_id, {"config": {"kind": "api"}})
    gpu_lock = MagicMock()
    build_target = Mock()
    monkeypatch.setattr(state, "gpu_lock", gpu_lock)
    monkeypatch.setattr(state, "build_target", build_target)
