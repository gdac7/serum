...
import { createRunSchema } from "./dto";
import { chatSchema } from "../../domain/dto";
...
// Mounted at /autodan by the approach registry, so every path here is relative.
export const autodanRouter = Router();
...
autodanRouter.post(
  ...
  async (req, res, next) => {
    try {
      const result = await runService.createRun(req.user!.id, "autodan", req.body);
...
autodanRouter.get("/runs/:id", requireAuth, async (req, res, next) => {
  try {
...
autodanRouter.get("/runs/:id/results", requireAuth, async (req, res, next) => {
  try {
...
autodanRouter.get("/runs/:id/prompts", requireAuth, async (req, res, next) => {
  try {
...
autodanRouter.get("/runs/:id/metrics", requireAuth, async (req, res, next) => {
  try {
...
autodanRouter.get("/runs/:id/progress", requireAuth, async (req, res, next) => {
  try {
...
autodanRouter.get("/runs/:id/events", async (req, res, next) => {
  let userId: string;
  try {
